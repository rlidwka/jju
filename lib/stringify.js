...
function _stringify(object, options, indentLvl) {
	...
	function _stringify_key(key) {
		if (options.quoteKeys) return _stringify_str(key)
	...
	function _stringify_object() {
		...
		if (options.indent && (len > 58 - indentLvl * 4 || len > 28) ) {
			...
			if (options.noTrailingComma && result.length) {
			...
			var innerStuff = result.join(options.indent ? ' ' : '').replace(/,$/, '')
			return braces[0]
				  + innerStuff
				  + braces[1]
		}
