...
function stringify_key(object, options) {
	if (String(Number(object)) == object && object[0] != '-') return object
	if (object == '') return stringify_str(object, options)

	var result = ''
	for (var i=0; i<object.length; i++) {
		if (i > 0) {
			if (!Uni.isIdentifierPart(object[i]))
				return stringify_str(object, options)

		} else {
			if (!Uni.isIdentifierStart(object[i]))
				return stringify_str(object, options)
		}

		var chr = object.charCodeAt(i)
...
function stringify_str(object, options) {
	...
	for (var i=0; i<object.length; i++) {
		...
		} else if (options.ascii || Uni.isLineTerminator(object[i]) || escapable.exec(object[i])) {
			if (chr < 0x100) {
				result += '\\x' + chr.toString(16)
		...
		case 'object':
			...
			if (Array.isArray(object)) {
				...
				for (var key in object) {
					...
					if (t !== undefined) {
						t = stringify_key(key, options) + ':' + (options.indent ? ' ' : '') + t + ','
						len += t.length + 1
						result.push(t)
					}
				}
