...
function formatError(input, msg, position, lineno, column) {
	...
	while (1) {
		...
		if (Uni.isLineTerminator(chr) || tmppos === input.length) {
			if (position >= tmppos) {
	...
	function parseString(endChar) {
		...
		while (position < length) {
			...
			if (chr === endChar) {
				...
				if (unescapeMap[chr] && (!legacy || unescapeMap[chr] != 'v')) {
...
module.exports.parse = function parseJSON(input, options) {
	...
	if (options.mode === 'json' || options.legacy) {
